      ...
      <el-row :gutter="10">
        ...
        <el-col :xs="20" :sm="10" :md="8" :lg="8" :xl="6">
          <div class="admin-logo-title">
            <h3>
              <b style="color:#1D8CE0">Estate</b>物业管理系统</h3>
          </div>
...
const validatorUtil = require("../../../utils/validatorUtil.js");//表单验证
...
export default {
  ...
  methods: {
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {//如果当前表单的状态合法提交这个表单
        if (valid) {
          //let params = this.adminLoginFormData;
          let params = Object.assign({}, this.adminLoginFormData);
          const password = params.password;
          params.password = crypto.MD5(params.password);//把密码进行md5加密，防止密码暴露

          api
            .post("admin/doLogin", params)//post请求提交
            .then(result => {
              if (result.data.state == "success") {
                window.location = "/manage";//登录成功跳转管理主页
              } else {//登录失败验证码重置$message
                this.reSetImgCode();
                this.$message({
